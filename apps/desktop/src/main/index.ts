...
function createWindow(): void {
  ...
  // Renderer diagnostics. When the renderer crashes hard enough that
  // DevTools can't be opened (white screen with no clickable surface),
  // the only way to recover the actual JS error is to forward it from the
  // main process. Without these, the user sees only the daemon-manager
  // polling noise (`Render frame was disposed before WebFrameMain could be
  // accessed`) which is a downstream symptom, not the cause.
  //
  // Dev mode: forward everything to the terminal. Production: log only
  // process crashes and preload failures — the signal-to-noise ratio of
  // full console forwarding is too low for production stderr, and we
  // can't ship a terminal with packaged builds anyway.
